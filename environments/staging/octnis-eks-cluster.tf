module "eks" {
  ...
  subnet_ids                      = ["${module.vpc-staging-asia.private_subnets_id[0]}", "${module.vpc-staging-asia.private_subnets_id[1]}"]
...
module "eks_blueprints_addons" {
  ...
  external_dns_route53_zone_arns = [data.aws_route53_zone.staging-hosted-zone.arn]
  external_dns = {
...
data "aws_route53_zone" "staging-hosted-zone" {
  name = "staging.uws-tech.com"
}
...
//In this example, Karpenter provisions nodes with the Bottlerocket v1.20.3 Amazon EKS optimized AMI (https://github.com/bottlerocket-os/bottlerocket)
...
//consolidateAfter: The amount of time Karpenter should wait after discovering a consolidation decision
//terminationGracePeriod: is the maximum time Karpenter will be draining a node before forcefully deleting, and it won’t wait for a replacement node after the node’s expiration has been met
