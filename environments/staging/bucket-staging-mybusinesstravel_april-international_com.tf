...
module "bucket-staging-mybusinesstravel_april-international_com" {
  source = "../../modules/s3"
     providers = {
    aws = aws.Paris
  }
  ...
  isProd = false
  addPolicy = true
  bucket-policy =  data.aws_iam_policy_document.bucket-policy-staging-mybusinesstravel_april-international_com.json
  tag = {
...
data "aws_iam_policy_document" "bucket-policy-staging-mybusinesstravel_april-international_com" {
statement {
